...
class Edit extends Component
{
    ...
    public function addPhoto()
    {
        try {
            ...
            Log::info('Foto guardada físicamente en m_Edit', [
                'path' => $path,
    ...
    {
        // Reducir la cantidad de datos cargados en móviles
    ...
    // Método para manejar la carga diferida de imágenes
    public function lazyLoadImages()
    {
        $this->dispatch('lazy-load-images');
    }
    ...
    // Método para ajustar el layout según el dispositivo
    public function adjustLayoutForDevice()
    {
        $device = $this->detectDevice();
        
        if ($device['isMobile']) {
            $this->dispatch('mobile-layout', [
                'singleColumn' => true,
                'stackedElements' => true,
                'compactSpacing' => true
            ]);
        } elseif ($device['isTablet']) {
            $this->dispatch('tablet-layout', [
                'twoColumns' => true,
                'mediumSpacing' => true
            ]);
        } else {
            $this->dispatch('desktop-layout', [
                'multiColumn' => true,
                'fullSpacing' => true
            ]);
        }
    ...
    public function handleOrientationChange()
    {
        $this->dispatch('orientation-change', [
            'portrait' => request()->header('X-Device-Orientation') === 'portrait',
            'landscape' => request()->header('X-Device-Orientation') === 'landscape'
        ]);
    }
    ...
    public function optimizeCache()
    {
        ...
        if ($device['isMobile']) {
            ...
            cache()->put('mobile_optimization_' . $this->serviceId, true, now()->addMinutes(30));
        }
    }

    // Método para comprimir imágenes en móviles
    public function compressImagesForMobile()
    {
        $device = $this->detectDevice();
        
        if ($device['isMobile']) {
            $this->dispatch('compress-images', [
                'quality' => 70,
                'maxWidth' => 800,
                'maxHeight' => 600
            ]);
    ...
    public function render()
    {
        ...
        // Filtrar usuarios para el modal - MOSTRAR TODOS LOS USUARIOS
        $filteredUsers = User::query()
            ...
            ->orderBy('name')
            ->limit($device['isMobile'] ? 20 : 50)
            ->get();

        // Filtrar inventarios para el modal con límites optimizados
        $filteredInventories = \App\Models\Inventory::with(['assignedUser', 'responsible'])
            ...
            ->when($this->inventorySearchUserName, function ($query) {
                $query->where(function ($subQuery) {
                    ...
                    ->orWhereHas('responsible', function ($userQuery) {
                        $userQuery->where('name', 'like', '%' . $this->userSearchUserName . '%');
                    });
                });
            })
            ...
            ->orderBy('ni')
            ->limit($device['isMobile'] ? 15 : 50)
            ->get();

        // Verificar configuración del servidor
        $serverWarnings = $this->checkServerConfiguration();

        return view('livewire.service.m_edit', [
            'users' => $users,
