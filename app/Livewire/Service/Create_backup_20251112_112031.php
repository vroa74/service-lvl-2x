...
class Create extends Component
{
    ...
    // --- INICIO: Propiedades para Inventarios Asociados ---
    ...
    // --- FIN: Propiedades para Inventarios Asociados ---
    ...
    public function selectInventory($inventoryId)
    {
        ...
        if ($this->inventoryModalType === 'objetivo') {
            $info = '<li>INVENTARIO: '
                . (!empty($inventory->type)     ? "Tipo: {$inventory->type}, "     : '')
    ...
    // --- INICIO: Métodos para Inventarios Asociados ---
    ...
    // --- FIN: Métodos para Inventarios Asociados ---
    ...
    public function addPhoto()
    {
        try {
            ...
            Log::info('Iniciando addPhoto en Create', [
            ...
            // Forzar el guardado del archivo con storeAs
            $filename = uniqid() . '_' . time() . '.' . $this->modalPhoto->getClientOriginalExtension();
            $path = $this->modalPhoto->storeAs('service_photos', $filename, 'public');
            
            Log::info('Foto guardada físicamente en Create', [
                'path' => $path,
            ...
            // Log para debugging
            Log::info('Foto agregada al array exitosamente en Create', [
                'path' => $path,
                'description' => $this->modalPhotoDescription,
            ...
            Log::error('Error de validación en addPhoto Create', [
                'errors' => $e->errors(),
                'modalPhoto' => $this->modalPhoto ? 'present' : 'null'
            ]);
            throw $e;
        } catch (\Exception $e) {
            Log::error('Error en addPhoto Create', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            session()->flash('error', 'Error al agregar foto: ' . $e->getMessage());
            $this->closePhotoForm();
        }
    }
    ...
    public function selectUser($userId, $userName)
    {
        ...
        if ($this->modalType === 'Solicitante') {
            ...
            $userInfo =
                (!empty(trim($miperfil->name))      ? "NOMBRE: {$miperfil->name}    "     : '') .
                (!empty(trim($miperfil->direction)) ? "DIRECCION: {$miperfil->direction}    " : '') .
                (!empty(trim($miperfil->position))  ? "CARGO: {$miperfil->position}"      : '');
            if (!empty($this->obj_sol)) {
                $this->obj_sol .= "\n" . $userInfo;
            } else {
    ...
    public function saveService()
    {
        ...
        try {
            ...
            Log::info('Fotos guardadas exitosamente', [
                'service_id' => $service->id,
                'total_photos' => count($this->servicePhotos)
            ]);

            // Sincronizar inventarios asociados
            if (!empty($this->selectedInventories)) {
                $inventoryData = [];
                ...
                Log::info('Inventarios sincronizados exitosamente', [
                    ...
                    'count' => count($this->selectedInventories)
                ]);
            }
    ...
    public function cleanupTemporaryPhotos()
    {
        foreach ($this->temporaryPhotos as $photoPath) {
            if (Storage::disk('public')->exists($photoPath)) {
                ...
                Log::info('Foto temporal eliminada', ['path' => $photoPath]);
            }
        }
        $this->temporaryPhotos = [];
    }
    ...
    {
        // NO limpiar fotos temporales aquí - solo se limpian cuando falla el guardado o se cancela
        // Las fotos temporales se convierten en permanentes al guardar el servicio exitosamente
        if (!empty($this->temporaryPhotos)) {
            Log::info('Fotos temporales pendientes en dehydrate (NO se eliminarán)', [
                'temporaryPhotos' => $this->temporaryPhotos,
                'totalServicePhotos' => count($this->servicePhotos)
            ]);
        }
    }
